  ...
  while ((choice = getopt_long(argc, argv, "m:h", long_options,
    ...
    case 'm':
      ...
      if (mode != "resize" && mode != "reserve" && mode != "nosize") {
        // The first line of error output has to be a 'fixed' message for the
        // autograder to show it to you.
        cerr << "Error: invalid mode" << endl;
        // The second line can provide more information, but you won't see it on
        // the AG.
...
int main(int argc, char *argv[]) {
  ...
  if (mode == "resize")
    ...
    cerr << "Invalid mode \"" << mode
         << "\" specified, should not be possible to reach this point!" << endl;
    return 1;
  } // if
