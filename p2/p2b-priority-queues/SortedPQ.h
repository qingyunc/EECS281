...
public:
  ...
  SortedPQ(InputIterator start, InputIterator end,
    ...
    updatePriorities(); // 1111111111111111111111111111111111
  }

  // Description: Destructor doesn't need any code, the data vector will
  //              be destroyed automatically.
  virtual ~SortedPQ() {} // ~SortedPQ()

  // Description: Add a new element to the PQ.
  // Runtime: O(n)
  virtual void push(const TYPE &val) {
    data.insert(std::lower_bound(data.begin(), data.end(), val, COMP_FUNCTOR()),
                val);
  }
  ...
  virtual void pop() { data.pop_back(); }
  ...
  //              element.
  // Runtime: O(1)
  virtual const TYPE &top() const { return data.back(); }
...
private:
  ...
  // TODO: Add any additional member fnctions you require here.
